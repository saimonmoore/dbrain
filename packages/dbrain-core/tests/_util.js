...
let nServer = 1
export async function createServer (extensions = 'dbrain-core') {
...
export class TestFramework {
  ...
  testFeed (t, entries, desc) {
    ...
    for (let i = 0; i < desc.length; i++) {
      this.testMemory(t, entries[i], desc[i])
    }
  }

  testMemory (t, entry, desc) {
    const user = desc[0]
  ...
  get allMemories () {
    return Object.values(this.users).map(user => user.memories).flat()
  }
  ...
  getRandomMemory () {
    ...
    return memories[randRange(0, memories.length - 1)]
  }
...
class TestCitizen {
  ...
  async createMemory ({link, text, title, type, excerpt, community}) {
    await this.login()
    const {url} = await this.inst.api.memories.create({link, text, title, type, excerpt, community})
    this.memories.push(await this.inst.api.memories.get(url))
